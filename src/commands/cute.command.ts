...
export const data = new SlashCommandBuilder()
    .setName("cute")
    .setDescription("Posts a random cute picture from the collected subreddits.");

export async function execute(interaction:ChatInputCommandInteraction) {
    await interaction.deferReply();
    // @ts-ignore
    const subreddit = await r.getSubreddit("AnimalsBeingBros+AnimalsBeingConfused+AnimalsBeingDerps+AnimalsBeingLoveable+aww+babyelephantgifs+NatureIsFuckingCute+NatureIsFuckingLit+PartyParrot+Pigtures+rarepuppers+SEUT+Sneks+thecatdimension");
    let count = 0;
